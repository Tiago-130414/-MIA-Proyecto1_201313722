...
string comando_rep::leerBloqueCarpeta(string pathDisco,int initPart,int indB){
  ...
  for(int i =0;i<4;i++){
      if(string(bc.b_content[i].b_name) != "." && string(bc.b_content[i].b_name) != ".." && bc.b_content[i].b_inodo != -1){
          cad += leerInodo(pathDisco,initPart,bc.b_content[i].b_inodo);
          cad += "bloque" + to_string(indB)+":"+to_string(bc.b_content[i].b_inodo) + "-> inodo" + to_string(bc.b_content[i].b_inodo)+ ":T"+ ";\n";
          //reporte += "inodo" + to_string(indI)+":"+to_string(indI) +"-> bloque" + to_string(inodoTemp.i_block[j]);
        }
    }
  return cad;
}

//metodo que genera tabla del bloque de archivos
string comando_rep::leerBloqueArchivo(string pathDisco,int initPart,int indB){
  bloqueArchivos bc = retornarBloqueArchivos(pathDisco,initPart,indB);
  string cad = recorrerBloqueArchivos(indB,bc);
  return cad;
}

string comando_rep::leerInodo(string pathDisco,int initPart,int indI){
  inodo inodoTemp = retornarInodo(pathDisco,initPart,indI);
  string reporte = recorrerInodos(indI,inodoTemp);
  for(int j = 0;j<12;j++){
      if(inodoTemp.i_block[j] != -1){
          if(inodoTemp.i_type == '0'){
              reporte += leerBloqueCarpeta(pathDisco,initPart,inodoTemp.i_block[j]);
              reporte += "inodo" + to_string(indI)+":"+to_string(inodoTemp.i_block[j]) +"-> bloque" + to_string(inodoTemp.i_block[j])+ ":T" + ";\n";
            }else if(inodoTemp.i_type == '1'){
              reporte += leerBloqueArchivo(pathDisco,initPart,inodoTemp.i_block[j]);
              reporte += "inodo" + to_string(indI)+":"+to_string(inodoTemp.i_block[j]) +"-> bloque" + to_string(inodoTemp.i_block[j])+ ":T"+ ";\n";
            }
        }
    }
...
string comando_rep::recorrerInodos(int indI,inodo inodoTemp){
  string reporte="";
  reporte += "inodo"+to_string(indI)+"[label=<\n";
  reporte += "<table border=\"0\" cellborder=\"1\" cellspacing=\"0\">\n";
  reporte += "<tr><td port = \"T\" bgcolor=\"blue\" colspan=\"2\"><i>Inodo</i></td></tr>\n";
  reporte += "<tr><td><i>NOMBRE</i></td><td><i>VALOR</i></td></tr>\n";
  reporte += "<tr><td>i_uid</td><td>"+ to_string(inodoTemp.i_uid) + "</td></tr>\n";
  reporte += "<tr><td>i_gid</td><td>"+ to_string(inodoTemp.i_gid) + "</td></tr>\n";
  reporte += "<tr><td>i_size</td><td>"+ to_string(inodoTemp.i_size) + "</td></tr>\n";
  ...
  for(int j = 0;j<15;j++){
      if(j<12){
          if(inodoTemp.i_block[j] != -1){
              reporte += "<tr><td>AD"+to_string(cont)+"</td><td port=\""+to_string(inodoTemp.i_block[j])+"\">"+ to_string(inodoTemp.i_block[j]) + "</td></tr>\n";
            }else{
              reporte += "<tr><td>AD"+to_string(cont)+"</td><td>"+ to_string(inodoTemp.i_block[j]) + "</td></tr>\n";
            }
        }
...
string comando_rep::recorrerBloqueCarpetas(int indB,bloqueCarpetas bc){
  string cad="";
  ...
  for(int i =0;i<4;i++){
      ...
      if(string(bc.b_content[i].b_name) != "." && string(bc.b_content[i].b_name) != ".." && bc.b_content[i].b_inodo != -1){
          cad += "<tr><td>"+string(bc.b_content[i].b_name)+"</td><td port=\""+to_string(bc.b_content[i].b_inodo)+"\">"+ to_string(bc.b_content[i].b_inodo) + "</td></tr>\n";
          // cout<<"INODO"<<to_string(bc.b_content[i].b_inodo)<<endl;
        }else{
          cad += "<tr><td>"+string(bc.b_content[i].b_name)+"</td><td>"+ to_string(bc.b_content[i].b_inodo) + "</td></tr>\n";
        }
      //cad += "<tr><td>"+string(bc.b_content[i].b_name)+"</td><td port=\""+to_string(bc.b_content[i].b_inodo)+"\">"+ to_string(bc.b_content[i].b_inodo) + "</td></tr>\n";
    }
...
string comando_rep::recorrerBloqueArchivos(int indB ,bloqueArchivos bc){
  string cad="";
  cad += "bloque"+to_string(indB)+"[label=<\n";
  cad += "<table border=\"0\" cellborder=\"1\" cellspacing=\"0\">\n";
